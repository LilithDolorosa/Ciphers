...
SaltMultiplier = 0
SaltCounter = 0
ListForSalting = list(CiphedText)
ListForSaltingLength = len(ListForSalting)
SaltPosition = SaltSpace * SaltMultiplier
print(ListForSalting)

while SaltPosition < ListForSaltingLength:
    if SaltCounter == 12:
        SaltCounter = 0
    SaltPosition = SaltSpace * SaltMultiplier
    Salt = str(SaltList[SaltCounter])
    ListForSalting.insert( SaltPosition, Salt)
    SaltMultiplier += 1
    SaltCounter += 1
...
input()
print(ListForSalting)
CiphedText = "".join(str(x) for x in ListForSalting)
input()
print(CiphedText)
#-------------------------------------------#
...
#ChangeLog
#0.01 -->Finished 25.05.17
#0.02 --> Adding basic salting. Shortened preview text
